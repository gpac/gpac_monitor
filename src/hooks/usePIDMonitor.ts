...
export const usePIDMonitor = () => {
  ...
  const bufferMetrics = useSelector((state: RootState) => state.pid.bufferMetrics);

  const updateIntervalRef = useRef<NodeJS.Timeout | null>(null);

  // Fonction pour calculer les métriques du buffer en évitant la division par zéro
  const calculateBufferMetrics = useCallback((buffer: number, total: number) => {
  ...
  const updatePIDData = useCallback(() => {
    ...
    const pidData = selectedPIDType === 'input' 
      ? selectedNode.ipid[selectedPID]
      : selectedNode.opid[selectedPID];

    if (!pidData) return;

    const newMetrics = calculateBufferMetrics(pidData.buffer, pidData.buffer_total);
  ...
  }, [selectedNode, selectedPID, selectedPIDType, dispatch, calculateBufferMetrics]);
