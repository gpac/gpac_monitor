...
export const Chart = memo(<T extends ChartDataPoint>({
  config,
  currentValue,
  isLive,
  createDataPoint
      ...
      <CardContent>
        <div style={{ width: "100%", height: mergedConfig.height }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={dataPoints} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              <defs>
                {mergedConfig.gradients?.map(gradient => (
                  <linearGradient key={gradient.id} id={gradient.id} x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={gradient.color} stopOpacity={gradient.opacity?.start || 0.6} />
                    <stop offset="95%" stopColor={gradient.color} stopOpacity={gradient.opacity?.end || 0.1} />
                  </linearGradient>
                ))}
              </defs>
