...
export const useVideoMonitor = (options: UseVideoMonitorOptions = {}) => {
    ...
    const selectedNode = useSelector((state: RootState) => state.graph.selectedFilterDetails);

    const calculateStats = useCallback((node: GpacNodeData): VideoStreamStats => {
        // Extract resolution
        ...
        const bufferPercentage = bufferTotal > 0 ? (bufferCurrent / bufferTotal) * 100 : 0;
      ...
      return {
        ...
        bufferStatus: stats.bufferHealth.percentage > 80 ? 'healthy' : 
                     stats.bufferHealth.percentage > 50 ? 'warning' : 'critical'
      };
    };