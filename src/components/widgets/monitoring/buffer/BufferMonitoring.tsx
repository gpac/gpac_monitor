...
const BufferStatus: React.FC<{
  ...
  const validBufferTotal = Number.isFinite(bufferTotal) && bufferTotal > 0 ? 
    bufferTotal : 0;

  const usage = isDynamic || validBufferTotal === 0 ? 
    0 : (validBuffer / validBufferTotal) * 100;
    ...
    return (
        <div className="flex items-center justify-between p-2 bg-gray-800/50 rounded">
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${getStatusColor(normalizedUsage)}`} />
            <span className="text-sm">{name}</span>
          </div>
          <div className="flex items-center gap-4">
            {!isDynamic && <div className="text-sm">{normalizedUsage.toFixed(1)}%</div>}
