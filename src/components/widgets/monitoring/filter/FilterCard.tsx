 ...
 */
const FilterCard: React.FC<FilterCardProps> = React.memo(({ filter, onClose }) => {
  return (
          ...
          <h3 className="font-medium text-lg flex items-center gap-2">
            <div 
              className={`w-2 h-2 rounded-full ${
                filter.nodeData.status?.includes('error') ? 'bg-red-500' :
                filter.nodeData.status?.includes('warning') ? 'bg-yellow-500' :
                'bg-green-500'
              }`}
            />
