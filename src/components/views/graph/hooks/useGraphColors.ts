...
const getNodeType = (node: Node): NodeType => {
  ...
  const hasOutputs = typeof nodeData.nb_opid === 'number' && nodeData.nb_opid > 0;
  const hasInputs = typeof nodeData.nb_ipid === 'number' && nodeData.nb_ipid > 0;
...
const getColorForNodeType = (nodeType: NodeType, theme: "dark" | "light"): string => {
  const hueRanges = {
  ...
  if (range.min <= range.max) {
    ...
    const wrap = Math.random() < ((360 - range.min) / (360 - range.min + range.max));
    hue = wrap
...
export const getImmediateGraphColor = (node: Node, theme?: "dark" | "light") => {
  const themeToUse = theme || (window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light");
  const nodeType = getNodeType(node);
