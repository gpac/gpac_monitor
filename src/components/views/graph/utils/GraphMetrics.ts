 ...
 */
export function calculateThroughputLevel(
  bytesPerSecond: number,
  filterType?: FilterType,
): ThroughputLevel {
 ...
 */
export function extractThroughputMetrics(
    ...
    Object.entries(filter.ipid).forEach(([pidName, pid]: [string, any]) => {
      ...
      if (filter.bytes_done && filter.last_ts && filter.start_ts) {
        const timeDiffSeconds =
          ((filter.last_ts as number) - (filter.start_ts as number)) / 1000000; // Convert to seconds
...
function determineConnectionType(
  sourceFilter: GpacNodeData,
  targetFilter: GpacNodeData,
): FilterType {
  ...
  if (sourceName.includes('video') || targetName.includes('video'))
    return 'video';
  if (sourceName.includes('audio') || targetName.includes('audio'))
    return 'audio';
  if (
    sourceName.includes('text') ||
    targetName.includes('text') ||
    sourceName.includes('subt') ||
    targetName.includes('subt')
  )
    return 'text';
  if (sourceName.includes('image') || targetName.includes('image'))
    return 'image';
 ...
 */
export function enhanceEdgesWithThroughput(
  edges: Edge[],
