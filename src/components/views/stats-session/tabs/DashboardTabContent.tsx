...
import { LuActivity, LuDatabase, LuArrowUp, LuArrowDown, LuPlay } from 'react-icons/lu';
import { FiltersGrid } from '../session-overview/FiltersGrid';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Progress } from '@/components/ui/progress';
import { ActivityIndicator, ActivityLevel } from '@/components/ui/activity-indicator';
...
const StatsCard: React.FC<StatsCardProps> = ({
  ...
  const getTrendIcon = () => {
    ...
    if (trend === 'down') return <LuArrowDown className="h-3 w-3 text-red-500" />;
    return null;
  };
  ...
  return (
    <Card className={`transition-all hover:shadow-md bg-black/50 border-gray-700 ${className}`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 p-2">
        <CardTitle className="text-xs font-medium text-muted-foreground">
          {title}
        </CardTitle>
        <div className="flex items-center gap-1">
          {activityLevel && <ActivityIndicator level={activityLevel} size="sm" />}
          {icon}
        </div>
      </CardHeader>
        ...
        {progress !== undefined && (
          <div className="mt-2">
            <Progress
              value={progress}
              className="h-1"
              color={progress > 80 ? 'bg-red-500' : progress > 50 ? 'bg-yellow-500' : 'bg-green-500'}
            />
...
export const DashboardTabContent: React.FC<DashboardTabContentProps> = ({
  ...
  const getSystemActivityLevel = (): ActivityLevel => {
    const processingRatio = statsCounters.total > 0 ? statsCounters.processing / statsCounters.total : 0;
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 justify-center max-w-4xl mx-auto">
          ...
          <StatsCard
            ...
            activityLevel={systemStats.totalBytes > 1000000 ? 'high' : systemStats.totalBytes > 100000 ? 'medium' : 'low'}
          />
          ...
          <StatsCard
            ...
            trend={statsCounters.sources > 0 && statsCounters.sinks > 0 ? 'up' : 'neutral'}
          />
        </div>
      </div>
