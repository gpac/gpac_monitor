...
export const MultimediaParams = memo(({ pidData }: MultimediaParamsProps) => {
  ...
  const formatFPS = (fps?: TimeFraction) => {
    if (!fps || typeof fps !== 'object' || !('num' in fps) || !('den' in fps) || fps.den === 0) return 'N/A';
  ...
  const getCodecBadgeVariant = (codec: string | undefined) => {
    ...
    if (lowerCodec.includes('h264') || lowerCodec.includes('avc')) return 'default';
    if (lowerCodec.includes('h265') || lowerCodec.includes('hevc')) return 'secondary';
  ...
  const hasVideoData = Object.values(videoParams).some(val => val !== undefined && val !== null);
  const hasAudioData = Object.values(audioParams).some(val => val !== undefined && val !== null);
  const hasTechnicalData = Object.values(technicalParams).some(val => val !== undefined && val !== null);
  ...
  const formatValue = (key: string, value: any) => {
    ...
    switch (key) {
      ...
      case 'bitrate':
        return typeof value === 'number' ? `${(value / 1000).toFixed(0)} kb/s` : String(value);
      case 'duration':
        return typeof value === 'number' ? `${(value / 1000).toFixed(2)}s` : String(value);
      case 'samplerate':
        return typeof value === 'number' ? `${(value / 1000).toFixed(1)} kHz` : String(value);
      case 'timescale':
  ...
  const renderParam = (label: string, key: string, value: any, showBadge: boolean = false) => {
    ...
    <Card className="bg-stat border-transparent">
      ...
      <CardContent className="p-3">
        <Accordion defaultExpanded={[]}>
          ...
          {hasVideoData && (pidData.width || pidData.height || pidData.pixelformat) && (
            <AccordionItem 
              value="video" 
            ...
            >
              <div className="space-y-1">
                ...
                {renderParam('Resolution', 'resolution', videoParams.resolution)}
                {renderParam('Pixel Format', 'pixelformat', videoParams.pixelformat)}
              </div>
            </AccordionItem>
          )}
          ...
          {hasAudioData && (pidData.samplerate || pidData.channels) && (
            ...
            >
              <div className="space-y-1">
                ...
                {renderParam('Sample Rate', 'samplerate', audioParams.samplerate)}
                ...
                {renderParam('Timescale', 'timescale', technicalParams.timescale)}
              </div>
            </AccordionItem>
          )}
        </Accordion>
