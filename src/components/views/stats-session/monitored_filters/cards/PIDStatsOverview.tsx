 ...
 */
export const PIDStatsOverview = memo(({ pidData, showAdvanced = true }: PIDStatsOverviewProps) => {

  // Determine status and variant based on real values
  const getStatusInfo = () => {
    if (pidData.stats.disconnected) return { status: 'Disconnected', variant: 'destructive' as const };
    if (pidData.would_block) return { status: 'Blocked', variant: 'destructive' as const };
    if (pidData.eos) return { status: 'End of Stream', variant: 'secondary' as const };
    if (pidData.playing) return { status: 'Playing', variant: 'default' as const };
    ...
    <div className="space-y-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <CardContent className="pt-0">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              ...
              <div className="text-xs text-muted-foreground stat-label">Buffer Used</div>
            </div>
            <div>
              <div className="text-sm font-medium">
              ...
              <div className="text-xs text-muted-foreground stat-label">Usage</div>
            </div>
            <div>
              <div className="text-sm font-medium">
      ...
      {showAdvanced && (
        <>
          ...
          {(pidData.stats.average_process_rate || pidData.stats.average_bitrate || pidData.stats.nb_processed) && (
            <PerformanceMetrics pidData={pidData} />
          )}

          {/* Multimedia Parameters - Show if we have codec/media data */}
          {(pidData.codec || pidData.width || pidData.samplerate || pidData.type) && (
