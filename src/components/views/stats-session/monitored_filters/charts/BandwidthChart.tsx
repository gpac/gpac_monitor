...
export const BandwidthChart = memo(({ currentBytes, type, refreshInterval = 5000 }: BandwidthChartProps) => {
  ...
  const chartTitle = useMemo(() => (type === "sent" ? "Upload Bandwidth" : "Download Bandwidth"), [type])
  const lineColor = useMemo(() => (type === "sent" ? "#10b981" : "#3b82f6"), [type])
    ...
    <Card className="bg-stat border-transparent">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-sm stat stat-label">
          {type === 'sent' ? <LuUpload className="h-4 w-4" /> : <LuDownload className="h-4 w-4" />}
          {chartTitle}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div style={{ width: "100%", height: 150 }}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={dataPoints} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
              ...
              <XAxis dataKey="time" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} minTickGap={40} tickMargin={5} />
              <YAxis {...chartComponents.yAxisProps} tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} />
              <Tooltip
