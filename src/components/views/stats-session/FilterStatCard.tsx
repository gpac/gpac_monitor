...
const cachedUsageCalculations = new WeakMap<GpacNodeData, { bufferUsage: number; activityLevel: string }>();

const FilterStatCard: React.FC<FilterStatCardProps> = memo(({ filter, onClick, isMonitored = false }) => {
  const { bufferUsage, activityLevel } = useMemo(() => {
    if (cachedUsageCalculations.has(filter)) {
      return cachedUsageCalculations.get(filter) as { bufferUsage: number; activityLevel: string };
    }

    const bufferUsage = calculateBufferUsage(filter.ipid);
    const activityLevel = getActivityLevel(filter.pck_done, filter.bytes_done);
  ...
  const statusVariant = filter.status?.includes('error') ? 'destructive' : 'secondary';
    ...
    >
      <CardHeader className="p-3 pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-medium truncate">{filter.name}</CardTitle>
          <div className="flex items-center gap-1 ml-2">
            {isMonitored && (
              <Badge variant="outline" className="flex h-5 items-center gap-1 px-1">
        ...
        <CardDescription className="mt-1 flex items-center gap-1 text-xs">
          ...
          <span className="truncate text-muted-foreground">{filter.status || 'No status'}</span>
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-3 p-3 pt-0">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-xs font-medium">{formatBytes(filter.bytes_done)}</p>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-1">
              <div className={`h-2 w-2 rounded-full ${getActivityColorClass(activityLevel)}`} />
              <span className="text-xs">{getActivityLabel(activityLevel)}</span>
            </div>
          </div>
        </div>
        ...
        {hasBufferInfo && (
          <div>
            <div className="mb-1 flex justify-between text-xs">
              <span className="text-xs text-muted-foreground">Buffer usage</span>
              <span className="text-xs">{bufferUsage}%</span>
            </div>
            <Progress value={bufferUsage} className="h-1.5" color={bufferColor} />
          </div>
        )}
