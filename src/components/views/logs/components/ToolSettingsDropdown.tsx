...
export const ToolSettingsDropdown = memo(function ToolSettingsDropdown({
  ...
  const handleLevelChange = useCallback((tool: GpacLogTool, level: GpacLogLevel) => {
    ...
    <DropdownMenu>
      ...
      >
        ...
        <DropdownMenuSeparator />
          ...
          <div className="absolute top-0 bottom-0 left-1/2 w-px bg-gray-600 transform -translate-x-1/2 z-5"></div>

          {/* Tools list */}
          {toolItems.map(({ tool, effectiveLevel, isCurrentTool, displayName, levelColor }) => (
            <div
              key={tool}
              className="flex items-center py-2 px-3 text-gray-200 text-small"
            >
              <span
