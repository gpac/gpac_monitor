...
import { GpacLogLevel, GpacLogTool, LogLevelUtils } from '@/types/domain/gpac/log-types';
...
describe('useLogsService - Intelligent Backend Call Optimization', () => {
  ...
  describe('LogLevelUtils', () => {
    it('should correctly determine when backend calls are needed', () => {
      ...
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.INFO, GpacLogLevel.DEBUG)).toBe(true);
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.WARNING, GpacLogLevel.INFO)).toBe(true);
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.ERROR, GpacLogLevel.WARNING)).toBe(true);

      // Test reduction (frontend filtering only)
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.DEBUG, GpacLogLevel.INFO)).toBe(false);
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.INFO, GpacLogLevel.WARNING)).toBe(false);
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.WARNING, GpacLogLevel.ERROR)).toBe(false);

      // Test same level (no backend needed)
      expect(LogLevelUtils.needsBackendCall(GpacLogLevel.INFO, GpacLogLevel.INFO)).toBe(false);
    });

    it('should correctly determine frontend filtering capability', () => {
      // Can use frontend filtering (requested <= current)
      expect(LogLevelUtils.canUseFrontendFiltering(GpacLogLevel.DEBUG, GpacLogLevel.INFO)).toBe(true);
      expect(LogLevelUtils.canUseFrontendFiltering(GpacLogLevel.INFO, GpacLogLevel.WARNING)).toBe(true);
      expect(LogLevelUtils.canUseFrontendFiltering(GpacLogLevel.INFO, GpacLogLevel.INFO)).toBe(true);

      // Cannot use frontend filtering (requested > current)
      expect(LogLevelUtils.canUseFrontendFiltering(GpacLogLevel.INFO, GpacLogLevel.DEBUG)).toBe(false);
      expect(LogLevelUtils.canUseFrontendFiltering(GpacLogLevel.WARNING, GpacLogLevel.INFO)).toBe(false);
    });
  });
  ...
  describe('Backend call optimization scenarios', () => {
    it('should skip backend call when reducing verbosity (mmio@info → mmio@warning)', async () => {
      ...
      store.dispatch(setToolLevel({ tool: GpacLogTool.MMIO, level: GpacLogLevel.INFO }));
      ...
      await act(async () => {
        store.dispatch(setToolLevel({ tool: GpacLogTool.MMIO, level: GpacLogLevel.WARNING }));
      });
    ...
    it('should call backend when increasing verbosity (mmio@info → mmio@debug)', async () => {
      ...
      await act(async () => {
        store.dispatch(setToolLevel({ tool: GpacLogTool.MMIO, level: GpacLogLevel.DEBUG }));
      });

      // Should call backend (needs more logs)
      expect(gpacService.logs.updateLogLevel).toHaveBeenCalledWith('mmio@debug');
    });
    ...
    it('should handle mixed scenarios: some need backend, some dont', async () => {
      ...
      store.dispatch(setDefaultAllLevel(GpacLogLevel.QUIET));
      store.dispatch(setToolLevel({ tool: GpacLogTool.MMIO, level: GpacLogLevel.INFO }));
      store.dispatch(setToolLevel({ tool: GpacLogTool.CODEC, level: GpacLogLevel.INFO }));
      ...
      await act(async () => {
        store.dispatch(setToolLevel({ tool: GpacLogTool.MMIO, level: GpacLogLevel.WARNING }));
        store.dispatch(setToolLevel({ tool: GpacLogTool.CODEC, level: GpacLogLevel.DEBUG }));
      });

      // Should only send the backend-required change
      expect(gpacService.logs.updateLogLevel).toHaveBeenCalledWith('codec@debug');
    ...
    it('should optimize multiple consecutive frontend-only changes', async () => {
      ...
      await act(async () => {
        store.dispatch(setToolLevel({ tool: GpacLogTool.FILTER, level: GpacLogLevel.WARNING }));
      });
      
      await act(async () => {
        store.dispatch(setToolLevel({ tool: GpacLogTool.FILTER, level: GpacLogLevel.ERROR }));
      });
    ...
    it('should call backend when escalating from frontend-filtered level', async () => {
      ...
      await act(async () => {
        store.dispatch(setToolLevel({ tool: GpacLogTool.NETWORK, level: GpacLogLevel.WARNING }));
      });
  ...
  describe('Real-world usage scenario', () => {
    it('should handle the exact scenario: filter@info → codec@warning/mutex@info → filter@info', async () => {
      ...
      expect(gpacService.logs.updateLogLevel).toHaveBeenCalledWith('codec@warning');
      vi.clearAllMocks();
      ...
      expect(gpacService.logs.updateLogLevel).toHaveBeenCalledWith('mutex@info');
      vi.clearAllMocks();
