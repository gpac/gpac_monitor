...
export const getPIDStatusBadge = (pid: TabPIDData) => {
  if (pid.stats.disconnected) return { text: 'Disconnected', variant: 'destructive' as const };
  if (pid.would_block) return { text: 'Blocked', variant: 'destructive' as const };
...
export const getGlobalStatus = (pids: TabPIDData[], itemCount: number) => {
  const totalErrors = pids.filter(pid => pid.stats.disconnected || pid.would_block).length;
