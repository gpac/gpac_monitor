...
interface GpacFilter {
  ...
  ipid: Record<
    string,
    { buffer: number; buffer_total: number; source_idx?: number }
  >;
  ...
  private startUpdates() {
    this.updateInterval = setInterval(() => {
      ...
      this.currentData = this.currentData.map((filter) => ({
        ...
        ipid: Object.entries(filter.ipid).reduce(
          (acc, [key, pid]) => ({
            ...acc,
            [key]: {
              ...pid,
              buffer: Math.min(
                100,
                Math.max(0, pid.buffer + (Math.random() * 20 - 10)),
              ),
            },
          }),
          {},
        ),
        opid: Object.entries(filter.opid).reduce(
          (acc, [key, pid]) => ({
            ...acc,
            [key]: {
              ...pid,
