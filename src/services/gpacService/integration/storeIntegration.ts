...
export const createStoreCallbacks = (): MessageHandlerCallbacks => ({
  ...
  onLogsUpdate: (logs: GpacLogEntry[]) => store.dispatch(appendLogsForAllTools(logs)),
  onLogSubscriptionChange: (isSubscribed: boolean) => store.dispatch(setSubscriptionStatus(isSubscribed)),
});
