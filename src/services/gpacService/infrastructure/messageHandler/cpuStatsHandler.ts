  ...
  private static generateMessageId(): string {
    ...
    public async subscribeToCPUStats(interval = 50): Promise<void> {
        ...
        if (this.pendingCPUStatsSubscribe) {
            console.log("CPU Stats subscribe request already in progress, reusing existing promise")
    ...
    public async unsubscribeFromCPUStats(): Promise<void> {
        ...
        if (this.pendingCPUStatsUnsubscribe) {
            console.log("CPU Stats unsubscribe request already in progress, reusing existing promise")
    ...
    public subscribeToCPUStatsUpdates(callback: (stats: CPUStats) => void, interval = 50): () => void {
        ...
        if (this.cpuAutoUnsubscribeTimeout) {
            ...
            console.log("Cancelled pending CPU auto-unsubscribe due to new subscriber")
        }
        ...
        if (isFirstSubscriber) {
            console.log("First subscriber for cpu stats, starting server subscription")
            this.subscribeToCPUStats(interval).catch((error) => {
                console.log(`Error subscribing to cpu stats on server: ${error}`, "stderr")
            })
        }
        ...
        return () => {
            ...
            if (!this.cpuStatsSubscribable.hasSubscribers) {
                console.log("No more listeners for cpu stats. Scheduling delayed auto-unsubscribe from server.") // Log
                ...
                this.cpuAutoUnsubscribeTimeout = setTimeout(() => {
                    ...
                    if (!this.cpuStatsSubscribable.hasSubscribers) {
                        ...
                        this.unsubscribeFromCPUStats().catch((error) => {
                            console.log(`Error unsubscribing from cpu stats on server: ${error}`, "stderr")
                        })
                    } else {
                        console.log("Cancelled cpu auto-unsubscribe - new subscribers detected") // Log
                    }
