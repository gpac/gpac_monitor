...
export class Subscribable<D, N = undefined, E extends object = ExtraData> {
  ...
  public get hasSubscribers() {
    return (
      ...
      Object.values(this.#channels).some((c) => Object.keys(c.subscribers).length > 0)
    )
  }

  public subscribe(subscriber: SubscriberFn<D, N, E>, options: Partial<SubscriberOptions> = {}) {
    const opts = {
      debounce: 150,
      immediate: true,
      ...options
    ...
    return () => {
      ...
      if (Object.keys(channel.subscribers).length === 0) delete this.#channels[opts.debounce]
    }
  }
  ...
  protected notify(type?: N, extraData?: Partial<E>) {
    ...
    if (type)
      Object.values(this.#channels).forEach((channel) => {
        channel.messages.push({ type, timestamp: performance.now(), extraData: extraData ?? {} })
      })
    ...
    Object.keys(this.#channels).forEach((debounce) => {
      ...
      channel.timeout = setTimeout(() => {
        ...
        const messages = channel.messages.filter((m) => m.timestamp > channel.lastSent)
        ...
        Object.values(channel.subscribers).forEach((subscriber) => {
          subscriber(
            ...
            messages.length === 0 ? undefined : messages.map((m) => m.extraData)
          )
        })
